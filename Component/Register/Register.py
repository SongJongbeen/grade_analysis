...
def register_exam():
    exam_folder = create_folder('./data', '모의고사')
    if exam_folder:
        exam_excel_path = exam_folder + f'/{exam_folder.lstrip("./data")} 정답 및 배점.xlsx'
...
def create_exam_excel(path):
    ...
    return save_excel_file(workbook, path, f'{path.lstrip("./data")} 정답 및 배점')


# 모의고사 폴더를 선택하여 학생들의 제출 답안 엑셀 파일을 생성하고, 파일을 엽니다.
def register_students_submission():
    exam_folder = select_exam_folder()
    submission_file = exam_folder + f'/{exam_folder.lstrip("./data")} 학생 제출 답.xlsx'
...
def create_students_submission_excel(exam_path):
    ...
    return save_excel_file(workbook, exam_path, f'{exam_path.lstrip("./data")} 학생 제출 답')
